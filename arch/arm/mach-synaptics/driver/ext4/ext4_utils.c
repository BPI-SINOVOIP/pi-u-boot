...
{
    unsigned char sha1[SHA1_DIGEST_LENGTH];
    struct ext4_uuid *uuid = (struct ext4_uuid *)result;

    sha1_hash(namespace, name, (unsigned char*)sha1);
    memcpy(uuid, sha1, sizeof(struct ext4_uuid));
...
void write_sb(struct blk_desc *dev, lbaint_t start, unsigned long long offset,
		struct ext4_super_block *sb)
{
	int ret;

	assert(offset % 512 == 0);
	assert(sizeof(*sb) / 512 == 0);

	ret = blk_dwrite(dev, start + offset / 512, sizeof(*sb) / 512, sb);
	if (ret < 0)
		critical_error_errno("failed to write superblock");
	if (ret != sizeof(*sb) / 512)
		critical_error("failed to write all of superblock");
}

static void block_device_write_sb(struct blk_desc *dev, lbaint_t start)
{
	...
	for (i = 1; i < aux_info.groups; i++) {
		if (ext4_bg_has_super_block(i)) {
			...
			write_sb(dev, start, offset, aux_info.backup_sb[i]);
		}
	}

	/* write out the primary superblock */
	write_sb(dev, start, 1024, aux_info.sb);
}

/* Write the filesystem image to a file */
void write_ext4_image(struct blk_desc *dev, lbaint_t start)
{
	sparse_file_write(ext4_sparse_file, dev, start);
	block_device_write_sb(dev, start);
}
...
void ext4_fill_in_sb(void)
{
	...
	for (i = 0; i < aux_info.groups; i++) {
		...
		if (ext4_bg_has_super_block(i)) {
			...
			sparse_file_add_data(ext4_sparse_file, aux_info.bg_desc,  BB_DATA_NOT_MALLOC,
...
{
	sparse_file_add_data(ext4_sparse_file, sb, BB_DATA_NOT_MALLOC, info.block_size, start_block);
}
...
void ext4_create_resize_inode()
{
	...
	if (inode == NULL) {
		printf("Error: failed to get resize inode");
		return;
	}
...
void ext4_create_journal_inode()
{
	...
	if (inode == NULL) {
		printf("Error: failed to get journal inode");
		return;
	}
	...
	u8 *journal_data = inode_allocate_data_extents(inode,
			...
			info.journal_blocks * info.block_size);
	if (!journal_data) {
		printf("Error: failed to allocate extents for journal data");
		return;
	}
