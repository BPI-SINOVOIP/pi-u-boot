...
static void reserve_indirect_block(struct block_allocation *alloc, int len)
{
	if (reserve_oob_blocks(alloc, 1)) {
		printf("Error: failed to reserve oob block");
		return;
	}

	if (advance_blocks(alloc, len)) {
		printf("Error: failed to advance %d blocks", len);
		return;
	}
}
...
static void fill_dindirect_block(u32 *dind_block, int len, struct block_allocation *alloc)
{
	...
	for (i = 0; len >  0; i++) {
		...
		sparse_file_add_data(ext4_sparse_file, ind_block_data, BB_DATA_MALLOC, info.block_size,
				ind_block);
		int ind_block_len = min((int)aux_info.blocks_per_ind, len);
		...
		if (advance_blocks(alloc, ind_block_len)) {
			printf("Error: failed to advance %d blocks", ind_block_len);
			return;
		}
...
static void fill_tindirect_block(u32 *tind_block, int len, struct block_allocation *alloc)
{
	...
	for (i = 0; len > 0; i++) {
		...
		sparse_file_add_data(ext4_sparse_file, dind_block_data, BB_DATA_MALLOC, info.block_size,
				dind_block);
		int dind_block_len = min((int)aux_info.blocks_per_dind, len);
...
{
	int len = min(*block_len, (u32)EXT4_NDIR_BLOCKS);
	int i;
		...
		struct block_allocation *alloc, u32 *block_len)
{
	...
	if (advance_oob_blocks(alloc, 1)) {
		printf("Error: failed to advance oob block");
		return -1;
	}
		...
		struct block_allocation *alloc, u32 *block_len)
{
	...
	sparse_file_add_data(ext4_sparse_file, tind_block_data, BB_DATA_MALLOC, info.block_size,
			tind_block);
...
static void reserve_all_indirect_blocks(struct block_allocation *alloc, u32 len)
{
	...
	printf("Error: %d blocks remaining", len);
}
		...
		struct block_allocation *alloc, u32 block_len)
{
	...
	if (inode_attach_direct_blocks(inode, alloc, &count)) {
		printf("Error: failed to attach direct blocks to inode");
		return -1;
	}

	if (count > 0) {
		if (inode_attach_indirect_blocks(inode, alloc, &count)) {
			printf("Error: failed to attach indirect blocks to inode");
			return -1;
		}
	}
	...
	if (count > 0) {
		if (inode_attach_dindirect_blocks(inode, alloc, &count)) {
			printf("Error: failed to attach dindirect blocks to inode");
			return -1;
		}
	}
	...
	if (count > 0) {
		if (inode_attach_tindirect_blocks(inode, alloc, &count)) {
			printf("Error: failed to attach tindirect blocks to inode");
			return -1;
		}
	}

	if (count) {
		printf("Error: blocks left after triply-indirect allocation");
		return -1;
	}
		...
		u32 block_len)
{
	...
	if (alloc == NULL) {
		printf("Error: Failed to allocate %d blocks", block_len + indirect_len);
		return NULL;
	}
...
void inode_allocate_indirect(struct ext4_inode *inode, unsigned long len)
{
	...
	if (alloc == NULL) {
		printf("Error: failed to allocate extents for %lu bytes", len);
		return;
	}
	...
	if (do_inode_attach_indirect(inode, alloc, block_len))
		printf("Error: failed to attach blocks to indirect inode");
		...
		struct block_allocation *alloc)
{
	...
	if (!ind_block_data)
		critical_error_errno("calloc");
	sparse_file_add_data(ext4_sparse_file, ind_block_data, BB_DATA_MALLOC,
